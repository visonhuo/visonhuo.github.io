...
## 前言
在微服务架构中，RPC、消息队列是我们常用的进程间通信的手段。
也正是由于这两种方式过于普及，导致我们很容易忽略其他进程间通信的交互模式。
...
因为确实 RPC 是最符合我们直观感受的进程间通信。其工作过程为：
...
简单来讲，进程间通信就是：**消息传递**，而消息传递的过程中，必然存在两个角色：**生产端** 和 **消费端** 。

如果我们不去关注消息底层究竟是如何进行传递的，那在整个过程中，我们只需要考虑如下三个维度：
- 消费端是否需要回复消息？
- 生产端是否需要等待回复消息？（**同步 or 异步**）
- 消息是发送给一个，还是多个消费者端？（**一对一 or 一对多**）

通过组合以上三个维度就可以得出以下几种进程间通信的交互模式。
|    |**一对一**|**一对多**｜
|:------:|:------:|:-------:|
|**同步模式**|同步请求/响应| 同步请求/多响应 |
|**异步模式**|异步请求/响应（有回复） <br /> 单向通知（无回复） | 发布/订阅（无回复）|

假设我们使用 RPC 来套用套用上述的三个维度：
...
接下来，我们将基于消息队列来逐个介绍一下以上列举的五种交互方式。

## 五种消息交互模式
### 同步请求/响应
{{< image src="https://i.bmp.ovh/imgs/2022/02/3590de50989105f5.png" caption="同步请求 + 一对一消费" >}}

这种便是典型的 RPC 工作模式。是的，通过消息队列我们同样可以实现 RPC 的工作方式，其中的关键点在于：
...
通过文字表述这个过程十分简单，但如果要将这套方案应用在生产环境中，则需要考虑很多细节。
在本人之前的工作经历中，确实存在某些部门是通过消息队列来实现 **同步请求/响应模式** 的。

相比于 RPC 方案，这种方式的好处在于：
- **统一组件** : 不需要单独维护另一套 RPC 方案，通过消息队列能同时满足多种交互模式；
- **通信可用性高** : 生产端和消费者端不一定需要同时都在线，可以允许短暂的节点失效；

而该方案的缺点则在于：
- **热点组件** : 消息队列承担了所有进程间通信的职责，需要组件本身具备高性能、高可用的特性；
- **消息延迟** : 因为所有的请求消息、回复消息都需要经过 MQ 才能传递到对端，所以一次请求的端到端延迟自然比直连的 RPC 通信高得多，尤其需要考虑队列负载很高时的情况。

{{< admonition type=tip title="代理型消息队列 or 无代理消息队列" open=open >}}
开源的 MQ 实现其实有很多，其中基于是否需要部署独立消息队列进程，可以分为如下两种类型。
- **代理型消息队列** : 需要部署独立的消息队列组件，所有的消息需要由队列进程进行传递。典型代表有 Apache Kafka、RabbitMQ 等；
- **无代理消息队列** : 不需要独立的消息队列组件，通过 Library 或者协议的方式工作，消息是在生产端和消费端之间直接传递。典型代表有 ZeroMQ 等；

如今代理型消息队列组件是使用更加广泛的一种方式，
所以本文中所有关于交互模式的介绍都是基于代理型消息队列，包括对其模式优缺点的讨论也都是建立在该基础之上。

需要注意 **代理型MQ** 和 **无代理MQ** 的工作机制是不同的，比如消息延迟高的缺点，
在无代理消息队列中并不是一个问题，因为它们是点对点直连的。
{{< /admonition >}}
...
所以请求发送邮件的 Server 并不需要关心邮件发送的执行结果，只需要把消息正常地投递出去即可。
...
从某种程度来说，**其破坏了邮件服务作为底层基础服务的通用性**。

{{< admonition type=question title="为什么消息类型很重要？" open=open >}}
如上所述，**消息类型决定了消息可用的交互模式，甚至决定了业务职责划分的问题**。
所以当我们在系统中设计消息相关的交互场景时，需要更加细致地考虑 **消息类型** 的选择问题。
{{< /admonition >}}
...
异步消息是我们日常开发中常用的手段，虽然我们熟悉它的使用，但也不免容易忽略使用手段背后应有的考量。
